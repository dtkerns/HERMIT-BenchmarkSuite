...
int main(int argc, char *argv[])
{
  ...
  for (i = 2; ++i < argc && *argv[i] == '-'; ) {
    switch (argv[i][1]) {
    case 'f': start = strtim(argv[++i]); break;
    case 't': end = strtim(argv[++i]); break;
    case 'l': length = strtim(argv[++i]); break;
    case 'h': hflag = 1; break;
    case 'm': mflag = 1; break;
    case 's': sflag = 1; break;
    case 'a': annotation = strann(argv[++i]); aflag = 1; break;
  ...
  while (getann(0, &annot[0]) >= 0) {
    if (isqrs(annot[0].anntyp))
      if ((!aflag && isqrs(annot[0].anntyp)) || (aflag && annot[0].anntyp == annotation))
        break;
  }

  while (getann(0, &annot[1]) >= 0 && (annot[1].time <= end || end == 0L)) {
    if ((!aflag && isqrs(annot[1].anntyp)) || (aflag && annot[0].anntyp == annotation && annot[1].anntyp == annotation)) {
      if (hflag)
      ...
      printf("%.3f %s\n", (annot[1].time - annot[0].time)/sps, annstr(annot[1].anntyp));
    }
